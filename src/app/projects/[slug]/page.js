      ...
      <div className='flex flex-col justify-between sm:col-span-5'>
        <div>
          ...
          <p className='pb-4 text-sm text-zinc-500'>
            منتشر شده در <time dateTime={`${year}-${month}-${day}`}>{`${year}/${month}/${day}`}</time>
          </p>
          <p className='pb-4'>{project.summary}</p>
        </div>
        <div className='grid grid-cols-1 gap-4 lg:grid-cols-2'>
          <Link href={project.preview_url || '#'} className='hover:no-underline' target='_blank'>
            <button
              disabled={project.preview_url ? false : true}
              className='btn flex w-full flex-row-reverse items-center justify-center gap-x-2 bg-blue-600 text-white hover:opacity-90 disabled:opacity-25'>
              پیش نمایش
              <svg
          ...
          <Link href={project.repo_url || '#'} className='hover:no-underline' target='_blank'>
            <button
              disabled={project.repo_url ? false : true}
              className='btn flex w-full flex-row-reverse items-center justify-center gap-x-2 bg-zinc-900 text-white hover:opacity-90 disabled:opacity-25'>
              گیت هاب
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='24'
